    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if group_lr not in [lr, 0.0]:
                raise RuntimeError(
                    f"Setting different lr values in different parameter groups is only supported for values of 0. To scale the learning rate differently for each layer, set the 'layer_scale' value instead."
                )
            for p in group["params"]:
                ...
                if "step" not in state:
                    ...
                    state["exp_avg"] = paddle.zeros_like(p.data).detach()
                    state["exp_avg_sq"] = paddle.zeros_like(p.data).detach()
        ...
        if sk_l1 == 0:
            return loss
        if fsdp_in_use:
            dist_tensor = paddle.zeros(2).cuda()
            dist_tensor[0] = numerator_acum
            dist_tensor[1] = sk_l1
            paddle.distributed.all_reduce(
                tensor=dist_tensor, op=paddle.distributed.ReduceOp.SUM
            )
            global_numerator_weighted = (
                sqrt_beta2 * numerator_weighted + (1 - sqrt_beta2) * dist_tensor[0]
            )
