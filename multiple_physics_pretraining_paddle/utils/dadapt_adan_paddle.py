        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                if not p.stop_gradient:
                    ...
                    state["exp_avg_diff"] = paddle.zeros_like(to_real(p.data)).detach()
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if "step" not in state:
                    ...
                    state["s"] = paddle.zeros_like(p.data).detach()
                    state["exp_avg"] = paddle.zeros_like(p.data).detach()
                    state["exp_avg_diff"] = paddle.zeros_like(p.data).detach()
                    state["exp_avg_sq"] = paddle.zeros_like(to_real(p.data)).detach()
                if state["step"] == 0:
                    state["pre_grad"] = grad.clone()
                exp_avg, exp_avg_diff, exp_avg_sq = (
                    state["exp_avg"],
                    state["exp_avg_diff"],
                    state["exp_avg_sq"],
                )
