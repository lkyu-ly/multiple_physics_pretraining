...
class SubsampledLinear(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        if self.subsample_in:
            scale = (
                self.dim_in / label_size
            ) ** 0.5  # Equivalent to swapping init to correct for given subsample of input
...
class hMLP_output(nn.Module):
    ...
    def __init__(self, patch_size=(16, 16), out_chans=3, embed_dim=768):
        ...
        self.out_proj = torch.nn.Sequential(
            *[
                nn.ConvTranspose2d(
                    embed_dim, embed_dim // 4, kernel_size=2, stride=2, bias=False
                ),
                RMSInstanceNorm2d(embed_dim // 4, affine=True),
                nn.GELU(),
                nn.ConvTranspose2d(
        ...
        out_head = nn.ConvTranspose2d(
            embed_dim // 4, out_chans, kernel_size=4, stride=4
        )
...
class AxialAttentionBlock(nn.Module):
    ...
    def forward(self, x, bcs):
        ...
        qx, kx, vx = map(
            lambda x: rearrange(x, "b he h w c ->  (b h) he w c"), [q, k, v]
        )
        ...
        qy, ky, vy = map(
            lambda x: rearrange(x, "b he h w c ->  (b w) he h c"), [q, k, v]
        )
