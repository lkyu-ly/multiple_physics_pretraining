...
class AViT(nn.Module):
    ...
    def expand_projections(self, expansion_amount):
        ...
        with torch.no_grad():
            ...
            out_head = nn.ConvTranspose2d(
                self.debed.embed_dim // 4,
                self.debed.out_chans + expansion_amount,
                kernel_size=4,
                stride=4,
            )
    ...
    def forward(self, x, state_labels, bcs):
        ...
        # Denormalize
        x = (
            x * data_std + data_mean
        )  # All state labels in the batch should be identical
